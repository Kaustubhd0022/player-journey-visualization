export default function StatsPanel({ events, selectedMatchId, matchDurationMs, playerTypeFilter, focusedPlayerId }) {
  const displayEvents = focusedPlayerId ? events.filter(e => e.user_id === focusedPlayerId) : events;
  
  const kills = displayEvents.filter(e => e.event === 'Kill' || e.event === 'BotKill').length;
  const deaths = displayEvents.filter(e => e.event === 'Killed' || e.event === 'BotKilled' || e.event === 'KilledByStorm').length;
  const loot = displayEvents.filter(e => e.event === 'Loot').length;
  const storm = displayEvents.filter(e => e.event === 'KilledByStorm').length;
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
        <StatBlock label={focusedPlayerId ? "PLAYER KILLS" : "TOTAL KILLS"} value={kills} color="var(--kill)" />
        <StatBlock label={focusedPlayerId ? "PLAYER STORM" : "STORM DEATHS"} value={storm} color="var(--storm)" />
        <StatBlock label={focusedPlayerId ? "PLAYER LOOT" : "ITEMS LOOTED"} value={loot} color="var(--loot)" />
